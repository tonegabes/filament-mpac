...
it('has expected cases and string values', function (): void {
    expect(RolePermissions::All->value)->toBe('roles.*')
        ->and(RolePermissions::ViewAny->value)->toBe('roles.view.any')
        ...
        ->and(RolePermissions::Replicate->value)->toBe('roles.replicate')
        ->and(RolePermissions::Reorder->value)->toBe('roles.reorder');
});

it('has exactly ten cases', function (): void {
    expect(RolePermissions::cases())->toHaveCount(10);
});
